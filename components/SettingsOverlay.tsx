...
interface SettingsOverlayProps {
    ...
    history?: MatchRecord[]; // Optional because it might not be passed in tests or strict contexts yet
}
...
export const SettingsOverlay: React.FC<ExpandedSettingsProps> = ({
    ...
    // Debug Filter State
    // Debug Filter State
    // const [debugFilter, setDebugFilter] = useState<'all' | 'log' | 'warn' | 'error'>('all'); // REMOVED
    ...
    // const [debugClicks, setDebugClicks] = useState(0); // REMOVED
    ...
    */
    ...
    const getHeroStats = (heroName: string) => {
        ...
        history.forEach(match => {
            ...
            const processPlayer = (p: { name: string, heroName: string }, won: boolean) => {
                if (p.heroName.trim().toLowerCase() === normalizedName) {
                    matches++;
                    if (won) wins++;
                }
            };
            match.team1.forEach(p => processPlayer(p, winner === 'team1'));
            match.team2.forEach(p => processPlayer(p, winner === 'team2'));
        });
        return { wins, matches };
    };

    const calculateRank = (wins: number, matches: number): string | null => {
    ...
    const getStatsEligibleHelpers = () => {
        ...
        currentHeroes.forEach(h => {
            const { matches } = getHeroStats(h.name);
    ...
    const confirmRankImport = () => {
        ...
        if (rankSourceType === 'list') {
            ...
            // Stats Import
            // Check for partial updates if not already confirmed
    ...
    const handleSwipe = () => {
        ...
        const tabs: TabType[] = ['lists', 'appearance', 'app_settings', 'info'];
    ...
    return (
        <div className={`fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950 flex flex-col transition-all duration-300 ease-in-out ${isOpen ? 'translate-x-0 opacity-100 visible' : 'translate-x-full opacity-0 invisible'}`}>
            ...
            <div className="flex-1 relative overflow-hidden" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
                {/* Editor Container */}
                {/* Editor Container Wrapper (Handles Animation + Positioning Context) */}
                <div className={`absolute inset-0 overflow-hidden transition-all duration-300 ease-out bg-slate-50 dark:bg-slate-950 ${editingListId ? 'translate-x-0 opacity-100 pointer-events-auto' : 'translate-x-[20%] opacity-0 pointer-events-none'}`}>
                    ...
                    {/* Custom Scrollbar - Sibling to scroller, absolute to wrapper */}
                    ...
                    <div className="pb-safe-area-bottom">
                        ...
                        {activeTab === 'info' && (
                            <div className="flex flex-col items-center justify-center h-full p-6 text-center animate-in fade-in slide-in-from-bottom-2">
                                ...
                                <div className="relative inline-block mb-8"> <p className="text-sm font-bold text-primary-500 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 px-3 py-1 rounded-full select-none">v2.7.0</p> </div>
