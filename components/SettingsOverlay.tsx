...
export const SettingsOverlay: React.FC<ExpandedSettingsProps> = ({
  ...
  // Debug Filter State
  const [debugFilter, setDebugFilter] = useState<'all' | 'log' | 'warn' | 'error' | 'info' | 'debug'>('all');
  ...
  // Re-implementing necessary helper functions for brevity as they haven't changed except UI render
  ...
  useEffect(() => {
    ...
    setEditorHeroes(prev => {
        if (prev.length === 0) return isReadOnly ? [] : [{ id: crypto.randomUUID(), name: '', rank: '' }];
        const lastIndex = prev.length - 1;
        const lastRow = prev[lastIndex];
        const isLastEmpty = lastRow.name.trim() === '' && lastRow.rank === '';
        if (isReadOnly) {
            if (isLastEmpty) return prev.slice(0, -1);
        } else {
            if (!isLastEmpty) return [...prev, { id: crypto.randomUUID(), name: '', rank: '' }];
        }
        return prev;
    });
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        const isInSettingsContext = currentState?.overlay === 'settings' || currentState?.overlay === 'settings-editor';
        if (!isInSettingsContext) window.history.pushState({ overlay: 'settings' }, '');
    }
  }, [isOpen]);
    ...
    if (isOpen) {
      const handlePopState = (event: PopStateEvent) => {
        ...
        if (isNameModalOpen || listToDelete || isDiscardModalOpen || isStatsModalOpen || importMode !== 'none' || isDebugExitModalOpen) {
            if (editingListId) window.history.pushState({ overlay: 'settings-editor' }, '');
            else window.history.pushState({ overlay: 'settings' }, '');
        ...
        if (overlay === 'settings-editor') return; 
  ...
  const validateRanks = (heroes: any[]): boolean => {
      return heroes.every(h => !h.rank || h.rank.trim() === '' || RANKS.includes(h.rank.trim()));
  };
  ...
  const handleToggleEditorMenu = (e: React.MouseEvent) => {
      e.stopPropagation();
      if (isEditorMenuOpen) setIsEditorMenuOpen(false);
      else {
  ...
  const handleToggleSort = () => { if (!sortLists) return; let nextOrder: SortOrder = sortOrder === 'custom' ? 'asc' : sortOrder === 'asc' ? 'desc' : 'asc'; setSortOrder(nextOrder); sortLists(nextOrder === 'desc' ? 'desc' : 'asc'); };
  ...
  const handleDiscardConfirm = () => {
      ...
      setEditingListId(null); setEditorHeroes([]); setEditorIsGroupable(false); setOriginalHeroesJson(''); setDiscardModalOpen(false); window.history.back(); 
  };
  ...
  const handleVersionClick = () => {
      ...
      setDebugClicks(prev => {
          const next = prev + 1;
          if (next >= 10) { if (onToggleDebug) onToggleDebug(true); return 10; }
          return next;
      });
  };
  const handleDebugOff = () => { setIsDebugExitModalOpen(true); };
  const confirmDebugOff = () => { if (onToggleDebug) onToggleDebug(false); setDebugClicks(0); setActiveTab('lists'); setIsDebugExitModalOpen(false); };
  const handleMouseDown = (e: React.MouseEvent) => { const el = tabsContainerRef.current; if (!el) return; setIsDragging(true); setIsDragScroll(false); setStartX(e.pageX - el.offsetLeft); setScrollLeft(el.scrollLeft); };
  ...
  const handleTouchEnd = (e: React.TouchEvent) => { if (editingListId || isListDragging) return; touchEndX.current = e.changedTouches[0].clientX; touchEndY.current = e.changedTouches[0].clientY; handleSwipe(); };
  const handleSwipe = () => {
      const SWIPE_THRESHOLD = 60;
      const diffX = touchStartX.current - touchEndX.current;
      const diffY = touchStartY.current - touchEndY.current;
      if (Math.abs(diffY) > Math.abs(diffX)) return;
      const tabs: TabType[] = isDebugMode ? ['lists', 'appearance', 'app', 'debug'] : ['lists', 'appearance', 'app'];
  ...
  const renderTabButton = (id: TabType, label: string, icon: React.ReactNode) => (
    <button
      data-tab-id={id}
      onClick={(e) => { if (isDragScroll) { e.preventDefault(); e.stopPropagation(); return; } setActiveTab(id); }}
      className={`shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-bold transition-all whitespace-nowrap select-none border ${activeTab === id ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900 border-transparent shadow-md' : 'bg-white dark:bg-slate-900 text-slate-500 border-slate-200 dark:border-slate-800'} ${isDragScroll ? 'pointer-events-none' : ''}`}
    >
  ...
  const handleDragEnter = (e: React.DragEvent<HTMLDivElement>, index: number) => {
    if (dragItem.current === null) return; dragOverItem.current = index;
    if (dragItem.current !== index && reorderLists) {
      const newLists = [...lists]; const draggedListContent = newLists[dragItem.current];
      newLists.splice(dragItem.current, 1); newLists.splice(index, 0, draggedListContent);
      dragItem.current = index; reorderLists(newLists); if (sortOrder !== 'custom') setSortOrder('custom');
    }
  };
  ...
  const handleTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    if (dragItem.current === null || !reorderLists) return; const touch = e.touches[0];
    const targetElement = document.elementFromPoint(touch.clientX, touch.clientY); const listItem = targetElement?.closest('[data-list-index]');
    if (listItem) { const index = parseInt(listItem.getAttribute('data-list-index') || '-1', 10); if (index !== -1 && index !== dragItem.current) { const newLists = [...lists]; const draggedListContent = newLists[dragItem.current]; newLists.splice(dragItem.current, 1); newLists.splice(index, 0, draggedListContent); dragItem.current = index; reorderLists(newLists); if (sortOrder !== 'custom') setSortOrder('custom'); } }
  };
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950 flex flex-col transition-all duration-300 ease-in-out ${isOpen ? 'translate-x-0 opacity-100 visible' : 'translate-x-full opacity-0 invisible'}`}>
        ...
        {activeTab === 'debug' && isDebugMode && (
            <div className="flex flex-col h-full bg-[#0d1117] text-gray-300 animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className="flex items-center gap-1 p-2 bg-[#0d1117] border-b border-gray-800 overflow-x-auto no-scrollbar">
                    ...
                    <button onClick={() => setDebugFilter('info')} className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold font-mono transition-colors border ${debugFilter === 'info' ? 'bg-cyan-900/30 text-cyan-400 border-cyan-900' : 'text-gray-500 border-transparent hover:bg-gray-900'}`}><Info size={10} /> INFO</button>
                    <button onClick={() => setDebugFilter('debug')} className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold font-mono transition-colors border ${debugFilter === 'debug' ? 'bg-purple-900/30 text-purple-400 border-purple-900' : 'text-gray-500 border-transparent hover:bg-gray-900'}`}><Bug size={10} /> DEBUG</button>
                ...
                <div className="flex-1 overflow-y-auto p-4 font-mono text-[11px] leading-relaxed">
                    {filteredLogs.length === 0 ? (
                        ...
                        filteredLogs.map((log, i) => (
                            <div key={i} className={`mb-2 font-mono break-all flex gap-2 group ${log.type === 'error' ? 'text-red-400' : log.type === 'warn' ? 'text-yellow-400' : log.type === 'info' ? 'text-cyan-400' : log.type === 'debug' ? 'text-purple-400' : 'text-gray-300'}`}>
            ...
            {activeTab === 'lists' && (
              <div className="animate-in fade-in slide-in-from-bottom-2">
                 <div className="flex items-center justify-between sticky top-0 z-30 px-4 pt-4 pb-4 bg-slate-50 dark:bg-slate-950">
                     <div className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-full border ${isOnline ? 'bg-green-100 text-green-700 border-green-200 dark:bg-green-500/20 dark:text-green-300 dark:border-green-500/30' : 'bg-slate-200 text-slate-500 border-slate-300'}`}> {isSyncing ? <><Loader2 size={10} className="animate-spin" /> Sync</> : isOnline ? <><Wifi size={10} /> Online</> : <><WifiOff size={10} /> Offline</>} </div>
                     <div className="flex gap-2"> <button onClick={handleToggleReorderMode} className={`w-9 h-9 flex items-center justify-center rounded-full border shadow-sm transition-colors ${isReorderMode ? 'bg-primary-100 text-primary-600 border-primary-200 dark:bg-primary-900/40 dark:text-primary-300 dark:border-primary-800' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}> <GripVertical size={18} /> </button> <button onClick={handleToggleSort} className="w-9 h-9 flex items-center justify-center rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm text-slate-600 dark:text-slate-300"> {sortOrder === 'desc' ? <ArrowUpAZ size={18} /> : <ArrowDownAZ size={18} />} </button> <button onClick={handleOpenCreate} className="h-9 px-4 flex items-center gap-2 bg-primary-600 text-white rounded-full shadow-md shadow-primary-600/20 active:scale-95 transition-transform"> <Plus size={18} /> <span className="text-sm font-bold">Новый</span> </button> </div>
                 </div>
                 <div className="px-4 pb-4"> {lists.length === 0 && <div className="text-center py-20 text-slate-400">Нет списков</div>} {lists.map((list, idx) => ( <ListItem key={list.id} list={list} index={idx} total={lists.length} isOnline={isOnline} contextMenuTargetId={contextMenuTargetId} onOpenMenu={handleOpenMenu} onEdit={handleOpenEditor} onDragStart={handleDragStart} onDragEnter={handleDragEnter} onDragEnd={handleDragEnd} isDragging={dragItem.current === idx} hasUpdate={updatedListIds ? updatedListIds.has(list.id) : false} onMarkSeen={onMarkSeen} isReorderMode={isReorderMode} /> ))} </div>
              </div>
            )}
            {activeTab === 'appearance' && (
              <div className="flex flex-col items-center justify-start min-h-full p-6 text-center animate-in fade-in slide-in-from-bottom-2">
                 <div className="w-full max-w-sm">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:gap-4"> {Object.entries(COLOR_SCHEMES_DATA).map(([key, data]) => { const isSelected = colorScheme === key; const colorValue = `rgb(${data.primary[500]})`; return ( <button key={key} onClick={() => setColorScheme && setColorScheme(key as any)} className={`relative flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-2xl border-2 transition-all duration-200 active:scale-95 ${isSelected ? 'border-primary-500 bg-white dark:bg-slate-800 shadow-md ring-2 ring-primary-500/20' : 'border-transparent bg-white dark:bg-slate-900 md:hover:bg-slate-50 dark:md:hover:bg-slate-800' } `} > <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full shrink-0 shadow-sm flex items-center justify-center" style={{ backgroundColor: colorValue }} > {isSelected && <Check size={20} className="text-white drop-shadow-md" />} </div> <div className="text-left min-w-0"> <div className={`text-xs sm:text-sm font-bold truncate ${isSelected ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'}`}> {data.label} </div> </div> </button> ); })} </div>
                    <div className="mt-8 p-4 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center"> <div className="text-center"> <div className="text-sm font-bold text-slate-900 dark:text-white mb-2">Пример кнопки</div> <button className="px-6 py-2 bg-primary-600 text-white font-bold rounded-xl shadow-lg shadow-primary-600/30"> Action </button> </div> </div>
                 </div>
              </div>
            )}
            {activeTab === 'app' && (
              <div className="flex flex-col items-center justify-center h-full p-6 text-center animate-in fade-in slide-in-from-bottom-2">
                 ...
                 <div onClick={handleVersionClick} className="relative cursor-pointer inline-block mb-8"> <p className="text-sm font-bold text-primary-500 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 px-3 py-1 rounded-full select-none">v2.2.1</p> {!isDebugMode && debugClicks > 5 && debugClicks < 10 && ( <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] font-bold text-slate-400"> Debug через {10 - debugClicks}... </div> )} </div>
      ...
      {/* Editor Menu Portal */}
      {isEditorMenuOpen && editorMenuRect && createPortal( <> <div className="fixed inset-0 z-[60] bg-transparent" onClick={() => setIsEditorMenuOpen(false)} /> <div className="fixed z-[61] w-52 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden animate-menu-in origin-top-right" style={{ top: editorMenuRect.bottom + 8, right: window.innerWidth - editorMenuRect.right, }} > <button onClick={() => handleEditorMenuAction(handleFileExport)} className="w-full text-left px-4 py-3 flex items-center gap-3 md:hover:bg-slate-50 dark:md:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-medium"> <FileJson size={16} /> Экспорт в файл </button> {!isReadOnly && !currentList?.isTemporary && ( <> <button onClick={() => handleEditorMenuAction(triggerFileUpload)} className="w-full text-left px-4 py-3 flex items-center gap-3 md:hover:bg-slate-50 dark:md:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-medium"> <Upload size={16} /> Импорт из файла </button> </> )} <div className="h-px bg-slate-100 dark:bg-slate-700 mx-2" /> <button onClick={() => handleEditorMenuAction(() => openTextExport(undefined))} className="w-full text-left px-4 py-3 flex items-center gap-3 md:hover:bg-slate-50 dark:md:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-medium"> <Copy size={16} /> Экспорт (Текст) </button> {!isReadOnly && !currentList?.isTemporary && ( <> <button onClick={() => handleEditorMenuAction(openTextImport)} className="w-full text-left px-4 py-3 flex items-center gap-3 md:hover:bg-slate-50 dark:md:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-medium"> <FileText size={16} /> Импорт (Текст) </button> <div className="h-px bg-slate-100 dark:bg-slate-700 mx-2" /> <button onClick={() => handleEditorMenuAction(openRankImport)} className="w-full text-left px-4 py-3 flex items-center gap-3 md:hover:bg-slate-50 dark:md:hover:bg-slate-700 text-violet-600 dark:text-violet-400 text-sm font-medium"> <ArrowLeftRight size={16} /> Импорт рангов </button> </> )} </div> </>, document.body )}

      {contextMenuTargetId && menuPosition && activeListForMenu && activeItemRect && createPortal( <> <div className="fixed inset-0 z-[60] bg-slate-900/20 backdrop-blur-[2px] animate-in fade-in duration-200" onClick={(e) => { e.stopPropagation(); handleCloseMenu(); }} /> <div onClick={() => handleCloseMenu()} className="fixed z-[61] bg-white dark:bg-slate-900 p-4 rounded-2xl flex items-center shadow-2xl ring-1 ring-slate-900/5 dark:ring-white/10 animate-in fade-in zoom-in-95 duration-200" style={{ top: activeItemRect.top, left: activeItemRect.left, width: activeItemRect.width, height: activeItemRect.height, transformOrigin: 'center center' }} > <div className="mr-4 ml-1 flex items-center justify-center w-10 h-10 rounded-full bg-slate-50 dark:bg-slate-800 shrink-0 border border-slate-100 dark:border-slate-700/50 relative"> {getListIcon(activeListForMenu)} {updatedListIds && updatedListIds.has(activeListForMenu.id) && ( <span className="absolute -top-1 -right-1 w-3 h-3 bg-blue-500 rounded-full border-2 border-white dark:border-slate-900"></span> )} </div> <div className="flex-1 mr-4"> <h3 className={`font-bold text-lg leading-tight mb-0.5 ${activeListForMenu.isTemporary ? 'text-primary-900 dark:text-primary-300 italic' : 'text-slate-900 dark:text-slate-100'}`}> {activeListForMenu.name} </h3> <p className="text-xs font-medium text-slate-500 dark:text-slate-500 flex items-center gap-2"> <span>Героев: {activeListForMenu.heroes.length}</span> {activeListForMenu.isTemporary && <span className="text-primary-500 dark:text-primary-400">временный</span>} {activeListForMenu.isCloud && !isOnline && <span className="px-1.5 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-500 text-[10px]">Offline</span>} </p> </div> <div className="p-2.5 rounded-xl bg-primary-50 text-primary-600 dark:bg-primary-900/30 dark:text-primary-300 shrink-0"> <MoreVertical size={20} /> </div> </div> <div className={`fixed z-[62] w-56 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden ${ menuPosition.origin === 'bottom' ? 'animate-menu-in-up origin-bottom-right' : 'animate-menu-in origin-top-right' }`} style={{ top: menuPosition.top, bottom: menuPosition.bottom, right: menuPosition.right }} > {!activeListForMenu.isCloud && !activeListForMenu.isTemporary && ( <button onClick={(e) => { e.stopPropagation(); handleUpload(activeListForMenu.id); }} disabled={!isOnline} className={`w-full text-left px-4 py-3.5 flex items-center gap-3 text-sm font-medium border-b border-slate-50 dark:border-slate-700/50 ${!isOnline ? 'opacity-50 cursor-not-allowed text-slate-400 dark:text-slate-500' : 'md:hover:bg-slate-50 dark:md:hover:bg-slate-700 text-sky-600 dark:text-sky-400'}`} > <UploadCloud size={16} /> Выгрузить в облако </button> )} {!activeListForMenu.isTemporary && ( <button onClick={(e) => { e.stopPropagation(); handleOpenRename(activeListForMenu); }} disabled={(!isOnline && activeListForMenu.isCloud)} className={`w-full text-left px-4 py-3.5 flex items-center gap-3 text-sm font-medium transition-colors ${(!isOnline && activeListForMenu.isCloud) ? 'opacity-40 cursor-not-allowed text-slate-400' : 'md:hover:bg-slate-50 dark:md:hover:bg-slate-700 text-slate-700 dark:text-slate-200'}`}> <Edit2 size={16} /> Переименовать </button> )} <button onClick={(e) => { e.stopPropagation(); openTextExport(activeListForMenu); }} className={`w-full text-left px-4 py-3.5 flex items-center gap-3 md:hover:bg-slate-50 dark:md:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-medium`}> <Copy size={16} /> Экспорт (Текст) </button> {!activeListForMenu.isTemporary && ( <button onClick={(e) => { e.stopPropagation(); handleExternalFileExport(activeListForMenu); }} className={`w-full text-left px-4 py-3.5 flex items-center gap-3 md:hover:bg-slate-50 dark:md:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-medium`}> <FileJson size={16} /> Экспорт в файл </button> )} <div className="h-px bg-slate-100 dark:bg-slate-700 mx-2" /> <button onClick={(e) => { e.stopPropagation(); handleDeleteClick(activeListForMenu); }} disabled={activeListForMenu.isCloud && !isOnline} className={`w-full text-left px-4 py-3.5 flex items-center gap-3 text-sm font-medium transition-colors ${activeListForMenu.isCloud && !isOnline ? 'opacity-40 cursor-not-allowed text-slate-400' : 'md:hover:bg-red-50 dark:md:hover:bg-red-900/20 text-red-600 dark:text-red-400'}`} > <Trash2 size={16} /> {activeListForMenu.isCloud ? 'Удалить из облака' : 'Удалить'} </button> </div> </>, document.body )}
      
      {/* ... MODALS ... */}
      <div className={`fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm transition-all duration-300 ${isNameModalOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}> <div className={`bg-white dark:bg-slate-900 w-full max-w-xs rounded-3xl p-6 shadow-2xl transition-all duration-300 border border-slate-100 dark:border-slate-800 ring-1 ring-slate-900/5 dark:ring-white/10 ${isNameModalOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}> <form onSubmit={handleNameSubmit}> <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">{nameModalMode === 'create' ? 'Новый список' : 'Переименовать'}</h3> <input autoFocus={isNameModalOpen} type="text" value={nameInputValue} onChange={(e) => setNameInputValue(e.target.value)} className="w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 rounded-xl mb-4 outline-none focus:ring-2 focus:ring-primary-500 text-slate-900 dark:text-white select-text" placeholder="Название..." /> {nameModalMode === 'create' && ( <div className="mb-6"> <div className="flex items-center gap-2 mb-2"> <div className="h-px flex-1 bg-slate-100 dark:bg-slate-800"></div> <span className="text-[10px] uppercase text-slate-400 font-bold">Или</span> <div className="h-px flex-1 bg-slate-100 dark:bg-slate-800"></div> </div> <button type="button" onClick={triggerCreateListFileUpload} className="w-full py-2.5 flex items-center justify-center gap-2 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-bold border border-slate-200 dark:border-slate-700 md:hover:bg-slate-100 dark:md:hover:bg-slate-700 transition-colors"> <Upload size={16} /> Загрузить из файла </button> <input type="file" ref={createListFileInputRef} className="hidden" accept=".json" onChange={handleNewListImport} /> </div> )} <div className="grid grid-cols-2 gap-3 mt-4"> <button type="button" onClick={handleCancelModal} className="py-3 font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-xl">Отмена</button> <button type="submit" disabled={!nameInputValue.trim()} className="py-3 font-bold text-white bg-primary-600 rounded-xl">ОК</button> </div> </form> </div> </div>
      <div className={`fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm transition-all duration-300 ${listToDelete ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}> <div className={`bg-white dark:bg-slate-900 w-full max-w-xs rounded-3xl p-6 shadow-2xl transition-all duration-300 border border-slate-100 dark:border-slate-800 ring-1 ring-slate-900/5 dark:ring-white/10 ${listToDelete ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}> <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Удалить?</h3> <p className="text-sm text-slate-500 mb-6">{isDeleteCloud ? 'Удалить из облака?' : 'Это действие необратимо.'}</p> <div className="grid grid-cols-2 gap-3"> <button onClick={handleCancelModal} className="py-3 font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-xl">Отмена</button> <button onClick={confirmDelete} className="py-3 font-bold text-white bg-red-500 rounded-xl">Удалить</button> </div> </div> </div>
      {/* ... Import/Export Modals (Text Export, Text Import, Rank Import, File Import Confirm) ... */}
      <div className={`fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm transition-all duration-300 ${importMode === 'text_export' ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`} onClick={() => setImportMode('none')}> <div className={`bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-6 shadow-2xl transition-all duration-300 border border-slate-100 dark:border-slate-800 ring-1 ring-slate-900/5 dark:ring-white/10 ${importMode === 'text_export' ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`} onClick={e => e.stopPropagation()}> <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Экспорт текста</h3> <p className="text-xs text-slate-500 mb-4">Формат: Имя|Ранг (одна строка - один герой)</p> <textarea value={importTextValue} readOnly className="w-full h-64 p-4 bg-slate-50 dark:bg-slate-950/50 rounded-xl text-xs font-mono border border-slate-200 dark:border-slate-700 outline-none focus:ring-2 focus:ring-primary-500 select-text mb-4" /> <div className="flex gap-3"> <button onClick={handleCopyText} className="flex-1 py-3 font-bold text-white bg-primary-600 rounded-xl">Копировать</button> <button onClick={() => setImportMode('none')} className="px-6 py-3 font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-xl">Закрыть</button> </div> </div> </div>
      <div className={`fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm transition-all duration-300 ${importMode === 'text_import' ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`} onClick={() => setImportMode('none')}> <div className={`bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-6 shadow-2xl transition-all duration-300 border border-slate-100 dark:border-slate-800 ring-1 ring-slate-900/5 dark:ring-white/10 ${importMode === 'text_import' ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`} onClick={e => e.stopPropagation()}> <div className="flex items-center gap-3 mb-2 text-orange-500"> <AlertCircle size={24} /> <h3 className="text-lg font-bold">Внимание!</h3> </div> <p className="text-sm text-slate-600 dark:text-slate-300 mb-4"> Текущий список героев будет <strong>полностью заменен</strong> данными из текстового поля. </p> <textarea value={importTextValue} onChange={(e) => setImportTextValue(e.target.value)} placeholder="Вставьте список героев (Имя|Ранг)" className="w-full h-48 p-4 bg-slate-50 dark:bg-slate-950/50 rounded-xl text-xs font-mono border border-slate-200 dark:border-slate-700 outline-none focus:ring-2 focus:ring-primary-500 select-text mb-6" /> <div className="grid grid-cols-2 gap-3"> <button onClick={() => setImportMode('none')} className="py-3 font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-xl">Отмена</button> <button onClick={confirmTextImport} className="py-3 font-bold text-white bg-orange-500 rounded-xl">Заменить</button> </div> </div> </div>
      <div className={`fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm transition-all duration-300 ${importMode === 'rank_import' ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`} onClick={() => setImportMode('none')}> <div className={`bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl p-6 shadow-2xl transition-all duration-300 border border-slate-100 dark:border-slate-800 ring-1 ring-slate-900/5 dark:ring-white/10 ${importMode === 'rank_import' ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`} onClick={e => e.stopPropagation()}> <div className="flex items-center gap-3 mb-2 text-violet-600 dark:text-violet-400"> <ArrowLeftRight size={24} /> <h3 className="text-lg font-bold">Импорт рангов</h3> </div> <p className="text-sm text-slate-600 dark:text-slate-300 mb-6"> Выберите список, из которого нужно скопировать ранги. Если имена совпадут, ранг текущего героя будет обновлен. </p> <div className="mb-6 relative"> <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Источник</label> <div className="relative"> <button onClick={() => setIsRankSourceDropdownOpen(!isRankSourceDropdownOpen)} className={`w-full p-3 flex items-center justify-between bg-slate-100 dark:bg-slate-800 rounded-xl border transition-all outline-none font-medium text-sm ${isRankSourceDropdownOpen ? 'border-violet-500 ring-1 ring-violet-500/20' : 'border-transparent'}`} > <span className={rankSourceListId ? 'text-slate-900 dark:text-white' : 'text-slate-400'}> {lists.find(l => l.id === rankSourceListId)?.name || "Выберите список..."} </span> <ChevronDown size={16} className={`text-slate-400 transition-transform duration-200 ${isRankSourceDropdownOpen ? 'rotate-180' : ''}`} /> </button> {isRankSourceDropdownOpen && ( <> <div className="fixed inset-0 z-10" onClick={() => setIsRankSourceDropdownOpen(false)} /> <div className="absolute top-full left-0 mt-2 w-full max-h-48 overflow-y-auto no-scrollbar bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 z-20 animate-in fade-in zoom-in-95 duration-200"> {lists.filter(l => l.id !== editingListId).length > 0 ? ( lists.filter(l => l.id !== editingListId).map(list => ( <button key={list.id} onClick={() => { setRankSourceListId(list.id); setIsRankSourceDropdownOpen(false); }} className={`w-full text-left px-4 py-3 text-sm font-medium transition-colors flex items-center justify-between ${rankSourceListId === list.id ? 'bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-300' : 'text-slate-700 dark:text-slate-200 md:hover:bg-slate-50 dark:md:hover:bg-slate-700'}`} > <span className="truncate">{list.name}</span> {rankSourceListId === list.id && <Check size={14} className="text-violet-500" />} </button> )) ) : ( <div className="px-4 py-6 text-center text-xs text-slate-400">Нет доступных списков</div> )} </div> </> )} </div> </div> <div className="grid grid-cols-2 gap-3"> <button onClick={() => setImportMode('none')} className="py-3 font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-xl">Отмена</button> <button onClick={confirmRankImport} disabled={!rankSourceListId} className="py-3 font-bold text-white bg-violet-600 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl">Импорт</button> </div> </div> </div>
      <div className={`fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm transition-all duration-300 ${importMode === 'file_import_confirm' ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}> <div className={`bg-white dark:bg-slate-900 w-full max-w-xs rounded-3xl p-6 shadow-2xl transition-all duration-300 border border-slate-100 dark:border-slate-800 ring-1 ring-slate-900/5 dark:ring-white/10 ${importMode === 'file_import_confirm' ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}> <div className="flex flex-col items-center text-center mb-6"> <div className="w-12 h-12 bg-orange-100 dark:bg-orange-900/30 text-orange-500 rounded-full flex items-center justify-center mb-4"> <AlertTriangle size={24} /> </div> <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Заменить список?</h3> <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed"> Текущие герои ({editorHeroes.length - (editorHeroes.some(h => !h.name) ? 1 : 0)}) будут заменены данными из файла ({pendingFileHeroes?.length}). </p> </div> <div className="grid grid-cols-2 gap-3"> <button onClick={() => { setImportMode('none'); setPendingFileHeroes(null); }} className="py-3 font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-xl">Отмена</button> <button onClick={confirmFileImport} className="py-3 font-bold text-white bg-orange-500 rounded-xl shadow-lg shadow-orange-500/20">Заменить</button> </div> </div> </div>
      
      {/* Editor Stats Modal */}
      <div className={`fixed inset-0 z-[70] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm transition-all duration-300 ${isStatsModalOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`} onClick={() => setIsStatsModalOpen(false)}> <div className={`bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl p-6 shadow-2xl transition-all duration-300 border border-slate-100 dark:border-slate-800 ring-1 ring-slate-900/5 dark:ring-white/10 max-h-[90dvh] flex flex-col ${isStatsModalOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`} onClick={(e) => e.stopPropagation()}> <div className="flex items-center justify-between mb-4"> <div className="flex items-center gap-3"> <div className="w-10 h-10 rounded-xl bg-violet-50 dark:bg-violet-900/30 flex items-center justify-center text-violet-600 dark:text-violet-400"> <BarChart3 size={20} /> </div> <h3 className="text-lg font-bold text-slate-900 dark:text-white">Баланс героев</h3> </div> <button onClick={() => setIsStatsModalOpen(false)} className="p-2 -mr-2 text-slate-400 md:hover:text-slate-900 dark:md:hover:text-white active:text-slate-900 rounded-full"> <X size={20} /> </button> </div> <div className="overflow-y-auto no-scrollbar flex-1 -mr-2 pr-2"> {(() => { const { counts, max, total } = getStats(); return ( <> <div className="text-xs font-medium text-slate-400 dark:text-slate-500 mb-4 text-center"> Всего героев: <span className="text-slate-900 dark:text-white font-bold">{total}</span> </div> {RANKS.map((rank, idx) => { const count = counts[rank] || 0; const percent = max > 0 ? (count / max) * 100 : 0; const colorClass = getRankBarColor(rank); return ( <div key={rank} className="mb-3 last:mb-0"> <div className="flex items-center justify-between text-xs font-bold text-slate-500 dark:text-slate-400 mb-1"> <span>{rank}</span> <span>{count}</span> </div> <div className="w-full h-2.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden"> <div className={`h-full rounded-full transition-all duration-1000 ease-out ${colorClass} ${percent === 0 ? 'opacity-0' : 'opacity-100'}`} style={{ width: isStatsModalOpen ? `${percent}%` : '0%', transitionDelay: `${idx * 50}ms` }} /> </div> </div> ); })} </> ); })()} </div> </div> </div>
