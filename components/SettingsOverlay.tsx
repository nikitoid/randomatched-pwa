...
export const SettingsOverlay: React.FC<ExpandedSettingsProps> = ({
  ...
  const handleDiscardConfirm = () => { if (editingListId && onDismissHeroUpdates) onDismissHeroUpdates(editingListId); setEditingListId(null); setEditorHeroes([]); setEditorIsGroupable(false); setOriginalHeroesJson(''); setDiscardModalOpen(false); window.history.back(); };
  const handleDiscardCancel = () => { setDiscardModalOpen(false); };

  const handleVersionClick = () => { if (isDebugMode) return; setDebugClicks(prev => { const next = prev + 1; if (next >= 10) { if (onToggleDebug) onToggleDebug(true); return 10; } return next; }); };
  ...
  const handleMouseMove = (e: React.MouseEvent) => { if (!isDragging || !tabsContainerRef.current) return; e.preventDefault(); const x = e.pageX - tabsContainerRef.current.offsetLeft; if (Math.abs((x - startX) * 2) > 5) setIsDragScroll(true); tabsContainerRef.current.scrollLeft = scrollLeft - (x - startX) * 2; };
  ...
  const handleSwipe = () => {
      ...
      const tabs: TabType[] = isDebugMode ? ['lists', 'appearance', 'app_settings', 'info', 'debug'] : ['lists', 'appearance', 'app_settings', 'info'];
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950 flex flex-col transition-all duration-300 ease-in-out ${isOpen ? 'translate-x-0 opacity-100 visible' : 'translate-x-full opacity-0 invisible'}`}>
          ...
          <div className="pb-safe-area-bottom">
            {activeTab === 'lists' && (
              <div className="animate-in fade-in slide-in-from-bottom-2">
                 ...
                 <div className="px-4 pb-4">
                    ...
                    {lists.map((list, idx) => ( <ListItem key={list.id} list={list} index={idx} total={lists.length} isOnline={isOnline} contextMenuTargetId={contextMenuTargetId} onOpenMenu={handleOpenMenu} onEdit={handleOpenEditor} onDragStart={handleDragStart} onDragEnter={handleDragEnter} onDragEnd={handleDragEnd} isDragging={dragItem.current === idx} hasUpdate={updatedListIds ? updatedListIds.has(list.id) : false} onMarkSeen={onMarkSeen} isReorderMode={isReorderMode} /> ))}
                 </div>
              </div>
            )}
            ...
            {activeTab === 'info' && (
              <div className="flex flex-col items-center justify-center h-full p-6 text-center animate-in fade-in slide-in-from-bottom-2">
                 ...
                 <div onClick={handleVersionClick} className="relative cursor-pointer inline-block mb-8"> <p className="text-sm font-bold text-primary-500 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 px-3 py-1 rounded-full select-none">v2.3.0</p> {!isDebugMode && debugClicks > 5 && debugClicks < 10 && ( <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] font-bold text-slate-400"> Debug через {10 - debugClicks}... </div> )} </div>
            ...
            {activeTab === 'debug' && isDebugMode && (
                <div className="flex flex-col h-full bg-[#0d1117] text-gray-300 animate-in fade-in slide-in-from-bottom-2">
                    <div className="flex items-center justify-between p-4 border-b border-gray-800 bg-[#161b22]"> <h3 className="text-sm font-bold text-white flex items-center gap-2 font-mono"><Terminal size={16} /> TERMINAL</h3> <button onClick={handleDebugOff} className="p-2 hover:bg-red-900/30 text-red-400 rounded-lg transition-colors"><Power size={16} /></button> </div>
                    <div className="flex items-center gap-1 p-2 bg-[#0d1117] border-b border-gray-800 overflow-x-auto no-scrollbar"> <button onClick={() => setDebugFilter('all')} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold font-mono transition-colors border ${debugFilter === 'all' ? 'bg-gray-800 text-white border-gray-600' : 'text-gray-500 border-transparent hover:bg-gray-900'}`}>ALL</button> <button onClick={() => setDebugFilter('log')} className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold font-mono transition-colors border ${debugFilter === 'log' ? 'bg-blue-900/30 text-blue-400 border-blue-900' : 'text-gray-500 border-transparent hover:bg-gray-900'}`}><Info size={10} /> LOG</button> <button onClick={() => setDebugFilter('warn')} className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold font-mono transition-colors border ${debugFilter === 'warn' ? 'bg-yellow-900/30 text-yellow-400 border-yellow-900' : 'text-gray-500 border-transparent hover:bg-gray-900'}`}><AlertTriangle size={10} /> WARN</button> <button onClick={() => setDebugFilter('error')} className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold font-mono transition-colors border ${debugFilter === 'error' ? 'bg-red-900/30 text-red-400 border-red-900' : 'text-gray-500 border-transparent hover:bg-gray-900'}`}><Bug size={10} /> ERROR</button> <div className="flex-1" /> <div className="text-[10px] text-gray-600 font-mono px-2">{filteredLogs.length} items</div> </div>
                    <div className="flex-1 overflow-y-auto p-4 font-mono text-[11px] leading-relaxed"> {filteredLogs.length === 0 ? ( <div className="h-full flex flex-col items-center justify-center text-gray-700 gap-2"> <Terminal size={32} className="opacity-20" /> <span>No output to display</span> </div> ) : ( filteredLogs.map((log, i) => ( <div key={i} className={`mb-2 font-mono break-all flex gap-3 group ${log.type === 'error' ? 'text-red-400' : log.type === 'warn' ? 'text-yellow-400' : 'text-gray-300'}`}> <span className="text-gray-600 shrink-0 select-none w-8 text-right opacity-50 group-hover:opacity-100 transition-opacity">{i + 1}</span> <div className="flex-1"> <div className="opacity-80">{log.args.join(' ')}</div> </div> </div> )) )} </div>
                </div>
            )}
          </div>
        </div>
      ...
      {/* (Other modals are identical to previous version, ensuring contextMenuTargetId portal is present) */}
      ...
      {/* (All other modals - Name, Delete, Import, Discard, Stats, Debug Exit - remain as is) */}
      ...
      <div className={`fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm transition-all duration-300 ${isDebugExitModalOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`} onClick={() => setIsDebugExitModalOpen(false)}> <div className={`bg-white dark:bg-slate-900 w-full max-w-xs rounded-3xl p-6 shadow-2xl transition-all duration-300 border border-slate-100 dark:border-slate-800 ring-1 ring-slate-900/5 dark:ring-white/10 ${isDebugExitModalOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}> <div className="flex flex-col items-center text-center mb-6"> <div className="w-12 h-12 bg-red-100 dark:bg-red-900/30 text-red-500 rounded-full flex items-center justify-center mb-4"><Power size={24} /></div> <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Отключить Debug?</h3> <p className="text-sm text-slate-500 dark:text-slate-400">Режим отладки будет скрыт.</p> </div> <div className="grid grid-cols-2 gap-3"> <button onClick={() => setIsDebugExitModalOpen(false)} className="py-3 font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-xl">Отмена</button> <button onClick={confirmDebugOff} className="py-3 font-bold text-white bg-red-500 rounded-xl">Отключить</button> </div> </div> </div>
      ...
      <div className={`fixed inset-0 z-[70] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm transition-all duration-300 ${isDiscardModalOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}> <div className={`bg-white dark:bg-slate-900 w-full max-w-xs rounded-3xl p-6 shadow-2xl transition-all duration-300 border border-slate-100 dark:border-slate-800 ring-1 ring-slate-900/5 dark:ring-white/10 ${isDiscardModalOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}> <div className="flex flex-col items-center text-center mb-6"> <div className="w-12 h-12 bg-orange-100 dark:bg-orange-900/30 text-orange-500 rounded-full flex items-center justify-center mb-4"><AlertTriangle size={24} /></div> <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Несохраненные изменения</h3> <p className="text-sm text-slate-500 dark:text-slate-400">Вы уверены, что хотите выйти? Изменения будут потеряны.</p> </div> <div className="grid grid-cols-2 gap-3"> <button onClick={handleDiscardCancel} className="py-3 font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-xl">Отмена</button> <button onClick={handleDiscardConfirm} className="py-3 font-bold text-white bg-orange-500 rounded-xl">Выйти</button> </div> </div> </div>
