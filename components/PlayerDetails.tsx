...
export const PlayerDetails: React.FC<PlayerDetailsProps> = ({
    ...
    const { recentMatches, topHeroes, partnerStats, bestStreak, worstStreak, totalKills, maxKills, matchesCount, winsCount, lossesCount, dynamicScore, levelInfo } = useMemo(() => {
        if (!playerName) {
            return {
                ...
                lossesCount: 0,
                dynamicScore: 0,
                levelInfo: calculatePlayerLevel(0, 0, 0)
            };
        }

        // Filter matches involving this player
        const playerMatches = history.filter(m =>
            m.team1.some(p => p.name === playerName) ||
            m.team2.some(p => p.name === playerName)
        ...
        playerMatches.forEach(m => {
            const isTeam1 = m.team1.some(p => p.name === playerName);
            const pData = isTeam1 ? m.team1.find(p => p.name === playerName) : m.team2.find(p => p.name === playerName);
        ...
        const weightedBreakdown = getPlayerWeightedBreakdown(playerName, history);
        ...
        playerMatches.forEach(m => {
            if (m.team1.some(p => p.name === playerName)) {
            ...
            if (m.team2.some(p => p.name === playerName)) {
    ...
    // Header title node with Avatar, name, inline editing, and match counts
    const headerTitle = (
            ...
            {isEditing ? (
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 flex items-center gap-1.5 mt-0.5">
                        <span>{formatPlural(matchesCount, 'игра', 'игры', 'игр')}</span>
    ...
    // Header actions: Level / Rank Badge
    const headerActions = (
        <button
            type="button"
            onClick={() => {
                triggerHaptic(10);
                setIsRanksInfoOpen(true);
            }}
        ...
        >
            ...
            <span className="text-[11px] font-bold tracking-wide hidden xs:inline">{levelInfo.tier.name}</span>
        </button>
    );
            ...
            >
                <div className="space-y-4 pb-2">
                                ...
                                <div className="pt-3 px-1.5 space-y-2">
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center gap-3 shrink-0">
                                                            ...
                                                            <div className="text-xs font-bold text-slate-500 flex items-center gap-1.5 w-24 justify-end">
                                                                ...
                                                                <span className="text-slate-500 dark:text-slate-400">{formatPlural(h.matches, 'игра', 'игры', 'игр')}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                );
                                    ...
                                    {topHeroes.length > 3 && (
                                        <button
                                            onClick={() => {
                                                triggerHaptic(10);
                                                setHeroesState(prev => prev === 'partial' ? 'expanded' : 'partial');
                                            }}
                    ...
                    <div className="bg-white dark:bg-slate-900 glass-card-gradient border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-4 shadow-xs">
                        ...
                        >
                            <div className="flex items-center gap-2">
                                <div className="p-1.5 rounded-lg bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-600 dark:indigo-400">
                                    <Users size={16} />
                                </div>
                                ...
                                <div className="pt-3 px-1.5 space-y-2">
                                    ...
                                    {partnerStats.length > 3 && (
                                        <button
                                            onClick={() => {
                                                triggerHaptic(10);
                                                setPartnersState(prev => prev === 'partial' ? 'expanded' : 'partial');
                                            }}
                                            className="w-full pt-2 text-xs font-bold text-primary-600 dark:text-primary-400 hover:underline transition-colors flex items-center justify-center gap-1 cursor-pointer"
                                        >
                                                ...
                                                >
                                                        ...
                                                        <div className="flex-1 text-right font-extrabold text-slate-800 dark:text-slate-200 truncate">
                                                            {myTeam.map((p, i) => (
                                                                <span
                                                                    key={i}
                                                                    onClick={(e) => {
                                                                        if (p.name !== player.name && onSelectPlayer) {
                                                                            e.stopPropagation();
                                                                            triggerHaptic(10);
                                                                            onSelectPlayer(p.name);
                                                                        }
                                                                    }}
                                                                    className={p.name === player.name ? 'text-primary-600 dark:text-primary-400 underline decoration-primary-500/40 font-black' : onSelectPlayer ? 'hover:underline cursor-pointer' : ''}
                                                                >
                                                    ...
                                                    {me && (
                                                        <div className="mt-2.5 pt-2 border-t border-slate-100 dark:border-slate-800/60 text-[11px] text-slate-500 dark:text-slate-400 flex justify-between items-center">
                                                            <span
                                                                className={`flex items-center gap-1 ${onSelectHero && me.heroName ? 'cursor-pointer hover:text-primary-600 dark:hover:text-primary-400 transition-colors' : ''}`}
                                                                onClick={(e) => {
                                                                    if (me.heroName && onSelectHero) {
                                                                        e.stopPropagation();
                                                                        triggerHaptic(10);
                                                                        onSelectHero(me.heroName);
                                                                    }
                                                                }}
                                                            >
                                                                <Shield size={12} className="text-slate-400" />
                                                                <span>Герой: <span className="font-bold text-slate-800 dark:text-slate-200 underline decoration-slate-300 dark:decoration-slate-700">{me.heroName}</span></span>
                                                            </span>
                                                            {me.kills !== undefined && me.kills !== null && (
                                                                <span className="px-2 py-0.5 rounded-full bg-rose-500/10 text-rose-500 font-extrabold text-[10px] flex items-center gap-1">
                                                                    <Skull size={11} /> {formatPlural(me.kills, 'убийство', 'убийства', 'убийств')}
                                                                </span>
                                                            )}
                                                        </div>
                                                    )}
...
export const PlayerDetailsModal = PlayerDetails;
